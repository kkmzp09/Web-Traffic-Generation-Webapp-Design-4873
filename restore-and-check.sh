...
# Restore from backup before the password reset changes
cp server-auth.js.backup-password-reset server-auth.js

# Now properly add CORS fix and password reset endpoints
cat > /tmp/auth-additions.js << 'EOF'
...
sed -i '/app.listen/r /tmp/auth-additions.js' server-auth.js

echo "✅ Password reset endpoints added"
