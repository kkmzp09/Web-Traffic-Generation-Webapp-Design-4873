...
# Restore to the backup before password reset
cp server-auth.js.backup-password-reset server-auth.js

# Create a proper ES module version of the password reset endpoints
cat > /tmp/password-reset-es.txt << 'ENDOFFILE'
...
sed -i '/app.listen/r /tmp/password-reset-es.txt' server-auth.js

echo "✅ Password reset endpoints added (ES module syntax)"
